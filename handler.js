...
export async function participantsUpdate({ id, participants, action }) {
    ...
    conn.sendButtonDoc(id, text, wm, action == 'add' ? 'ᴡᴇʟᴄᴏᴍᴇ' : 'sᴀʏᴏɴᴀʀᴀᴀ', action === 'add' ? '.intro' : 'ImOscar', ftoko, { contextInfo: { externalAdReply: { showAdAttribution: true,
    mediaUrl: "https://youtu.be/KzU_d0ctB3o",
    mediaType: 2, 
    title: action === 'add' ? 'Semoga Betah ツ' : 'Selamat Tinggal ツ',
    body: wm,
    thumbnail: await(await fetch(action === 'add' ? wel : lea)).buffer(),
    sourceUrl: 'https://youtu.be/KzU_d0ctB3o',
     }}
  })
                    }
                }
